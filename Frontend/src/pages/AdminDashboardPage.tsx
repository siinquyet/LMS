...
export const AdminDashboardPage: React.FC = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			setLoading(true);
			try {
				const [overviewData, analyticsData, coursesData, ordersData] =
					await Promise.all([
						getAdminOverview(),
						getAnalytics(),
						getCourses(),
				...
				setOverview(overviewData?.overview || { users: 0, courses: 0, revenue: 0, pending_courses: 0 });
	...
	const totalRevenue = successOrders.reduce(
		(sum: number, o: any) => sum + o.tong_tien,
		0,
	);
	const monthlyRevenue = successOrders
		.filter((o: any) => o.ngay_dat?.startsWith("2024-07"))
		.reduce((sum: number, o: any) => sum + o.tong_tien, 0);
			...
			<div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
				<div className="flex gap-4 lg:gap-6 overflow-x-auto pb-2">
					...
					<Card className="min-w-[200px] p-6 flex-1">
						...
						<p className="font-['Inter', sans-serif] text-3xl text-[#1C293C]">
							{formatPrice(overview?.revenue || 0)}
						</p>
					</Card>
					<Card className="min-w-[200px] p-6 flex-1">
						<div className="flex items-center gap-3 mb-2">
					...
					<Card className="p-6">
						...
						<div className="space-y-3">
							{pendingCourses.length === 0 ? (
								<p className="font-['Inter', sans-serif] text-gray-500 text-sm">
								...
								pendingCourses.slice(0, 4).map((course: any) => (
									...
									>
										...
										<div className="flex-1 min-w-0">
											<h3 className="font-['Inter', sans-serif] text-[#1C293C] text-sm truncate">
												{course.tieu_de}
											</h3>
					...
					<Card className="p-6">
						...
						<div className="space-y-3">
							{recentOrders.map((order: any) => (
								...
								>
									<div>
										<h3 className="font-['Inter', sans-serif] text-[#1C293C] text-sm">
											#{order.id} - {order.items?.[0]?.khoa_hoc?.tieu_de}
										</h3>
										<p className="font-['Inter', sans-serif] text-xs text-gray-500">
									...
									<div className="text-right">
										...
										<Badge
											variant={
												order.trang_thai === "success" ? "success" : "default"
											}
										>
											{order.trang_thai}
										</Badge>
