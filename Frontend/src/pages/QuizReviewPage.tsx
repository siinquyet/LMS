	...
	return (
		<div className="min-h-screen bg-[#F8F6F3]">
			...
			<div className="bg-white border-b-2 border-[#1C293C] px-4 py-3 flex items-center justify-between sticky top-0 z-10">
				<Link
					to={`/quiz/${courseId}/${lessonId}/result`}
				...
				<div className="flex items-center gap-2">
					...
					<span className="font-['Inter', sans-serif] text-red-500">
						{result.total - result.score} câu sai
					</span>
				</div>
			</div>
					...
					return (
						...
						>
							...
							<div className="flex items-center gap-2 mb-3">
								...
								<span
									className={`font-['Inter', sans-serif] text-sm ${isCorrect ? "text-green-600" : "text-red-500"}`}
								>
									{isCorrect ? "Đúng" : "Sai"}
								</span>
							...
							{/* Question */}
							<h3 className="font-['Inter', sans-serif] text-[#1C293C] mb-3">
								{q.question}
							</h3>
							...
							<div className="space-y-2">
								{q.options.map((option, optIdx) => {
									...
									return (
										<div
											key={optIdx}
											className={`p-3 rounded-[8px] text-sm font-['Inter', sans-serif] ${
												isCorrectOption
