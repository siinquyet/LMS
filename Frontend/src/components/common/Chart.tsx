...
export const Chart: React.FC<ChartProps> = ({
			...
			<div className="relative h-full">
				...
				<div className="absolute bottom-0 left-0 right-0 flex justify-between">
					{data.map((item, idx) => (
						<span
							key={idx}
							className="font-['Comfortaa', cursive] text-xs text-[#263D5B]"
						>
							{item.label}
						</span>
					))}
				...
				<div className="relative w-32 h-32">
					<svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
						{data.map((item, idx) => {
							...
							const endX =
								50 + 40 * Math.cos((Math.PI * (startAngle + angle)) / 180);
							const endY =
								50 + 40 * Math.sin((Math.PI * (startAngle + angle)) / 180);
	...
	return (
		<div
			className={`bg-white border-2 border-[#263D5B] rounded-[12px] p-4 ${className}`}
			style={{ height }}
		>
